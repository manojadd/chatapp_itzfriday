...
export default class ChatHistory extends Component {
	...
	render() {
		...
		let messageList = this.props.chatHistory.map((message,i)=>{
			...
			return (<Row key={i} start="xs"><Col xs={10} style={{marginTop:'2vh',marginBottom:'2vh',maxWidth:'80%'}}><Card >
				<CardTitle title={message.sender} subtitle={message.TimeStamp}  />
         			<CardText>{message.msg}</CardText>
					</Card></Col></Row>);
			else
			return (<Row key={i} end="xs"><Col xs={10} style={{marginTop:'2vh',marginBottom:'2vh',maxWidth:'80%'}}><Card >
				<CardTitle title={message.sender} subtitle={message.TimeStamp}  />
         			<CardText>{message.msg}</CardText>
					</Card></Col></Row>);
